...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        in_width    = x.shape[2]
        in_height   = x.shape[3]
        f_height    = self.W.shape[2]
        f_width     = self.W.shape[3]

        out_width  = in_width - f_width + 1
        out_height = in_height - f_height + 1

        win_shape = (1, 1, f_width, f_height)
        windows   = view_as_windows(x,win_shape)
        
        windows        = windows.reshape(batch_size,in_ch_size,out_width,out_height,f_width,f_height)
        forward_result = torch.tensordot(windows, self.W, dims=([1,4,5],[1,2,3]))
        forward_result = torch.permute(forward_result,(0,3,1,2))

        forward_result += self.b
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        out_ch_size = in_ch_size

        win_shape   = (batch_size, in_ch_size, pool_size, pool_size)
        windows = view_as_windows(x, win_shape, step=stride)
             
        windows = torch.permute(windows,(4,5,2,3,0,1,6,7))
        windows = windows.reshape(batch_size, out_ch_size, out_width, out_height,pool_size,pool_size)
        windows = windows.reshape(batch_size, out_ch_size, out_width, out_height, -1)
        
        forward_result, _ = torch.max(windows, dim=-1)
